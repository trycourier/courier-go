...
func TestWorkspacePreferenceListLogsWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.WorkspacePreferences.ListLogs(context.TODO(), courier.WorkspacePreferenceListLogsParams{
		Cursor:   courier.String("cursor"),
		Limit:    courier.Int(1),
		Since:    courier.String("since"),
